...
import ListingAvailabilityFields from '@/components/listing-editor/ListingAvailabilityFields.vue';
...
const selectedCategoryId = ref(
    property.value?.service_category_id ||
        listing.value?.service_category_id ||
        categories.value[0]?.id ||
        '',
);
const selectedSubCategoryId = ref(
    property.value?.service_sub_category_id ||
        listing.value?.service_sub_category_id ||
        '',
);

const availableSubCategories = computed(() =>
    subCategoriesFor(categories.value, selectedCategoryId.value),
);
const selectedSubCategory = computed<ServiceSubCategory | undefined>(() =>
    availableSubCategories.value.find(
        (item) => item.id === selectedSubCategoryId.value,
    ),
);
const isPropertyFlow = computed(
    () => selectedSubCategory.value?.default_listing_table === 'properties',
);
const isEditing = computed(() => Boolean(property.value || listing.value));
const isPriceOptional = computed(
    () =>
        !isPropertyFlow.value &&
        ['artisan', 'professional'].includes(
            selectedSubCategory.value?.provider_kind || '',
        ),
);
const dynamicValues = ref<FieldValue[]>(
    collection(property.value?.field_values || listing.value?.field_values),
);
const existingImages = ref(
    collection(property.value?.images || listing.value?.images),
);
...
const form = useForm({
    ...
    description:
        property.value?.description || listing.value?.description || '',
    ...
    base_price:
        property.value?.base_price || listing.value?.pricing?.amount || '',
    maximum_amount: listing.value?.pricing?.maximum_amount || '',
    currency:
        property.value?.currency || listing.value?.pricing?.currency || 'NGN',
    pricing_unit:
        property.value?.pricing_unit ||
        listing.value?.pricing?.billing_period ||
        '',
    billing_period:
        listing.value?.pricing?.billing_period ||
        property.value?.pricing_unit ||
        '',
    owner_phone:
        property.value?.owner_phone || listing.value?.contact?.phone || '',
    contact_name: listing.value?.contact?.name || '',
    contact_phone:
        listing.value?.contact?.phone || property.value?.owner_phone || '',
    preferred_contact_method:
        listing.value?.contact?.preferred_method || 'phone',
    ...
    availability_slots:
        property.value?.availability_slots ||
        listing.value?.availability_slots ||
        [],
    availability_rules:
        property.value?.availability_rules ||
        listing.value?.availability_rules ||
        [],
...
<template>
    <form
        class="grid min-w-0 gap-6 lg:grid-cols-[minmax(0,1fr)_360px]"
        @submit.prevent="submit"
    >
        <section class="min-w-0 space-y-5 bg-white p-1">
            <CategorySelector
            ...
            <ListingAvailabilityFields :form="form" />
