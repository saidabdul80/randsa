...
const activeTab = ref<'housing' | 'services'>(
    initialListing ? 'services' : 'housing',
);
const activeModal = ref<'create' | 'edit-property' | 'edit-listing' | null>(
    initialProperty ? 'edit-property' : initialListing ? 'edit-listing' : null,
);
...
const activeCollection = computed(() =>
    activeTab.value === 'housing'
        ? props.properties
        : props.marketplaceListings,
);
const activeCount = computed(() =>
    activeTab.value === 'housing'
        ? properties.value.length
        : marketplaceListings.value.length,
);
const columns = [
...
<template>
    ...
    >
        <div class="grid gap-4 sm:grid-cols-3">
            ...
            <div
                class="rounded-[18px] border border-emerald-100 bg-emerald-50/60 p-5"
            >
                <p
                    class="text-[11px] font-black tracking-[0.18em] text-emerald-900 uppercase"
                >
                    Create
                </p>
                <p class="mt-3 text-sm leading-6 text-slate-600">
                    Post housing, artisan services, rentals, and marketplace
                    listings from one configured form.
                </p>
                <button
                    type="button"
        ...
        >
            <div
                class="flex flex-col gap-4 border-b border-slate-100 pb-4 sm:flex-row sm:items-end sm:justify-between"
            >
                <div>
                    <p
                    ...
                    <p class="mt-1 text-sm text-slate-500">
                        {{ activeCount }} records on this page
                    </p>
                </div>
            ...
            <div class="-mx-4 mt-4">
                ...
                >
                    ...
                    >
                        ...
                        >
                            {{
                                property.sub_category?.label ||
                                statusLabel(property.service_sub_category_id)
                            }}
                        </td>
                        <td
                            class="px-5 py-4 text-sm font-semibold text-slate-700"
                        ...
                        <td class="px-5 py-4 text-right">
                            <div class="inline-flex gap-2">
                                <Link
                                    ...
                                    class="grid h-10 w-10 place-items-center rounded-[10px] border border-slate-200 text-slate-700 transition hover:bg-slate-50"
                                ...
                                <button
                                    type="button"
                                    class="grid h-10 w-10 place-items-center rounded-[10px] bg-slate-950 text-white transition hover:bg-slate-800"
                    ...
                    >
                        ...
                        >
                            {{
                                ...
                                statusLabel(listing.service_sub_category_id)
                            }}
                        </td>
                        <td
                            class="px-5 py-4 text-sm font-semibold text-slate-700"
                        ...
                        <td class="px-5 py-4">
                            <StatusBadge
                                :status="
                                    listing.moderation_status || listing.status
                                "
                            />
                        </td>
                        <td class="px-5 py-4 text-sm text-slate-600">
        ...
        <AppModal
            ...
            :title="
                activeModal === 'create' ? 'Create listing' : 'Update listing'
            "
            eyebrow="Publishing"
            size="xl"
            @close="closeModal"
