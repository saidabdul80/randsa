...
export function resolveListingQuality(value: PropertyFormInput): ListingQualityResult {
  ...
  const hasCommercialAccess = Boolean(
    value.roadAccess || value.marketArea || value.electricityAvailability || value.waterAccess || value.security,
  )

  const checks: ListingQualityCheck[] = [
    { id: 'title', label: 'Clear listing title', complete: value.title.trim().length >= 8, blocking: !value.title.trim() },
    { id: 'description', label: 'Useful description', complete: value.description.trim().length >= 80, blocking: !value.description.trim() },
    { id: 'pricing', label: 'Rental price', complete: value.rentPrice > 0, blocking: value.rentPrice <= 0 },
    { id: 'location', label: 'Complete address', complete: hasCompleteLocation, blocking: !hasCompleteLocation },
    { id: 'map', label: 'Accurate map pin', complete: hasCoordinates, blocking: false },
    { id: 'phone', label: 'Contact phone', complete: Boolean(value.ownerPhone.trim()), blocking: !value.ownerPhone.trim() },
    { id: 'cover', label: 'Cover image', complete: value.images.length > 0, blocking: value.images.length === 0 },
    { id: 'gallery', label: 'Three or more images', complete: value.images.length >= 3, blocking: false },
    { id: 'amenities', label: 'Useful amenities', complete: value.amenities.length >= 2, blocking: false },
  ]
  ...
  if (config.detailMode === 'commercial') {
    ...
    checks.push(
      ...
      { id: 'commercial-access', label: 'Access and utilities', complete: hasCommercialAccess, blocking: false },
    )
  }
