<template>
  <nav class="overflow-x-auto" aria-label="Notification filters">
    <div
      class="flex min-w-max items-center gap-1 border-b border-slate-200 px-4 sm:px-5 dark:border-slate-800"
    >
      <button
      ...
      >
        ...
        <span
          ...
          class="ml-1 text-xs"
          :class="
            modelValue === filter.value ? 'text-brand-600 dark:text-brand-300' : 'text-slate-400'
          "
        >
          ({{ filter.count }})
        </span>
