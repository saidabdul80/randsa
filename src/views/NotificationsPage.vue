  ...
  >
    ...
    <header class="mb-6 flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
      <div class="flex min-w-0 items-start gap-4">
        <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-brand-50 text-brand-600 dark:bg-brand-500/15 dark:text-brand-300">
        ...
        <div class="min-w-0">
          ...
          <p class="mt-1 max-w-2xl text-sm leading-6 text-slate-600 dark:text-slate-300">
            Stay close to booking confirmations, payment updates, inspection reminders, and browser alerts.
          </p>
        </div>
      </div>
      ...
      <div class="flex w-full items-center gap-2 xl:max-w-md">
        ...
        <button
          ...
          @click="handleRefresh"
        >
          <IonIcon :icon="refreshOutline" class="text-xl" :class="isRefreshing ? 'animate-spin' : ''" />
        </button>
      </div>
    </header>
    ...
    <div
      ...
      :class="messageTone === 'success' && !pageError
    ...
    <section class="grid items-start gap-5 xl:grid-cols-[minmax(0,2fr)_minmax(280px,0.9fr)]">
      <div class="overflow-hidden rounded-[20px] border border-slate-200 bg-white shadow-[0_16px_40px_-30px_rgba(16,32,51,0.45)] dark:border-slate-800 dark:bg-slate-900">
        <div class="flex items-center justify-between gap-4 px-4 pb-2 pt-5 sm:px-5">
          ...
          <IonIcon :icon="optionsOutline" class="text-xl text-slate-400" aria-label="Filter options below" />
        </div>

        <NotificationFilters v-model="activeFilter" :filters="availableFilters" />

        <div v-if="isInitialLoading" class="divide-y divide-slate-100 dark:divide-slate-800" aria-label="Loading notifications">
        ...
        <div v-else-if="filteredNotifications.length" class="divide-y divide-slate-100 dark:divide-slate-800">
          <NotificationItem
      ...
      <aside class="grid gap-4 xl:sticky xl:top-5">
        <section class="overflow-hidden rounded-[20px] bg-brand-600 p-6 text-white shadow-[0_18px_36px_-24px_rgba(31,111,231,0.75)]">
          <div class="flex items-start justify-between gap-4">
            ...
            <div class="relative flex h-14 w-14 items-center justify-center rounded-2xl bg-white/15">
          ...
          <p class="mt-4 text-sm leading-6 text-brand-50">
            {{ unreadCount ? 'You have notifications waiting for your attention.' : 'You are all caught up.' }}
          </p>
          <button
            type="button"
        ...
        <section class="rounded-[20px] border border-slate-200 bg-white p-5 shadow-[0_14px_32px_-28px_rgba(16,32,51,0.45)] dark:border-slate-800 dark:bg-slate-900">
          <div class="flex items-center justify-between gap-3">
            <p class="text-xs font-bold text-brand-700 dark:text-brand-300">Push readiness</p>
            <span
              class="inline-flex items-center gap-1 text-xs font-semibold"
              :class="tokens.length ? 'text-emerald-600 dark:text-emerald-300' : 'text-slate-500 dark:text-slate-400'"
            >
              <IonIcon :icon="tokens.length ? checkmarkCircleOutline : alertCircleOutline" aria-hidden="true" />
          ...
          <div v-if="tokens.length" class="mt-4 grid gap-3">
            ...
            >
              <summary class="flex cursor-pointer list-none items-center justify-between gap-3 text-sm font-bold text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 dark:text-white">
                <span class="min-w-0 truncate">{{ formatDeviceName(token.device) }}</span>
                <IonIcon :icon="chevronDownOutline" class="shrink-0 transition group-open:rotate-180" aria-hidden="true" />
              </summary>
              <div class="mt-3 border-t border-slate-200 pt-3 text-xs leading-5 text-slate-500 dark:border-slate-700 dark:text-slate-400">
                <p class="font-semibold text-slate-700 dark:text-slate-200">Technical details</p>
          ...
          >
            ...
            {{ activeAction === 'push' ? 'Enabling...' : tokens.length ? 'Register this device' : 'Enable push notifications' }}
          </button>
        </section>

        <section class="rounded-[20px] border border-emerald-100 bg-emerald-50/70 p-5 dark:border-emerald-500/20 dark:bg-emerald-500/[0.08]">
          ...
          <p class="mt-3 text-sm leading-6 text-slate-700 dark:text-slate-300">
            Inspection reminders run automatically on a schedule. You can also check for reminders now.
          </p>
          <button
            type="button"
        ...
        <section class="rounded-[20px] border border-violet-100 bg-violet-50/65 p-5 dark:border-violet-500/20 dark:bg-violet-500/[0.08]">
          ...
          <div class="mt-3 grid gap-2 text-sm leading-6 text-slate-700 dark:text-slate-300">
            <p>Inspection reminders are generated when a booking is within the next 24 hours.</p>
...
const pageError = computed(() => loadError.value || (messageTone.value === 'error' ? message.value : ''))
